...
def main() -> int:
    ...
    print(f"Acurácia institucional | {len(itens)} perguntas | modelo={cfg.modelo_llm}\n")
    ...
    for item in itens:
        ...
        recuperados = {c.id for c in r.trechos}
        gold = relevancia[item.id]
        hit = bool(recuperados & gold)
        recusou = _recusou(r.texto)
        cit_ok = bool(set(r.fontes) & gold)
        ...
        linhas_csv.append({
            ...
            "fontes_citadas": " ".join(map(str, r.fontes)),
    ...
    n = len(itens)
    print(f"recuperação (chunk gold no top-{cfg.top_k_contexto}): {n_hit}/{n} = {n_hit/n:.0%}")
