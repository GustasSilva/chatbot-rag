...
class RespostaDialogo:
    ...
    origem: Origem
    trechos: tuple[Chunk, ...]  # fontes citáveis (vazio quando não se entendeu)
...
class Dialogo:
    ...
    def responder(self, pergunta: str) -> RespostaDialogo:
        ...
        return self._recorrer_ao_plano_b(pergunta)

    def _recorrer_ao_plano_b(self, pergunta: str) -> RespostaDialogo:
        if self._plano_b is None:
            return RespostaDialogo(pergunta, NAO_ENTENDI, Origem.NAO_ENTENDIDA, ())
        resposta = self._plano_b.responder(pergunta)
        return RespostaDialogo(
            pergunta=pergunta,
